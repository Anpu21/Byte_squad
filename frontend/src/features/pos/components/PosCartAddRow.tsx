...
export function PosCartAddRow({
    ...
    const results = useMemo<IProduct[]>(
        () => (inventoryQuery.data?.items ?? []).map((inv) => inv.product),
        [inventoryQuery.data],
    );
    ...
    return (
        <tr className="bg-surface-2/30 border-t border-border-strong">
            ...
            <td
                ref={cellRef}
                className="relative px-2 py-2 align-middle"
            >
                <div className="flex items-center gap-1">
                    <input
                        ...
                        aria-label="Add product by barcode or name"
                        aria-autocomplete="list"
                        aria-expanded={showDropdown}
                        className="flex-1 min-w-0 h-8 px-2 bg-canvas border border-border rounded-md text-[12px] text-text-1 outline-none focus:border-primary focus:ring-[2px] focus:ring-primary/30 transition-all placeholder:text-text-3"
                    />
                    <button
                        type="button"
                        ...
                        className="h-8 w-8 flex-shrink-0 bg-surface border border-border rounded-md flex items-center justify-center text-text-2 hover:text-text-1 hover:border-border-strong hover:bg-surface-2 transition-all focus:outline-none focus:ring-[2px] focus:ring-primary/20"
                    >
                ...
                {showDropdown && (
                    <div
                        ...
                        className="absolute left-0 right-0 bottom-full mb-1 z-20 bg-surface border border-border rounded-md shadow-md-token min-w-[280px] max-h-[260px] overflow-y-auto"
                    >
                        {results.length === 0 ? (
                            ...
                            results.map((product, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <span className="text-[12px] font-semibold text-text-1 tabular-nums mono flex-shrink-0">
                                            {formatCurrency(
                                                Number(product.sellingPrice),
                                            )}
                                        </span>
                                        {active && (
                                            <kbd className="hidden sm:inline-flex items-center justify-center h-5 px-1.5 rounded border border-border bg-surface-2 text-[10px] font-bold text-text-2 flex-shrink-0">
                                                Enter
                                            </kbd>
                                        )}
                                    </button>
