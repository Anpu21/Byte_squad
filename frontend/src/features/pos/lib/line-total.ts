 ...
 *   3. `lineSubtotal = chargedQty × unitPrice × (1 - discount/100)`.
...
export function computeLine(input: IComputeLineInput): ILineMath {
    ...
    const lineSubtotal = round2(
        chargedQty * input.unitPrice * (1 - input.discountPercentage / 100),
    );
    const lineDiscountAmount = round2(
        chargedQty * input.unitPrice * (input.discountPercentage / 100),
    );
