/**
 * The inter-service contract between this backend (publisher) and the
 * ledgerpro-realtime service (subscriber). The backend PUBLISHes a
 * `RealtimeEnvelope` as JSON on REALTIME_CHANNEL; the subscriber maps `target`
 * to a socket.io room and emits `event` with `payload` on `namespace`.
 *
 * Keep this file byte-identical with the realtime repo's copy — a mismatch
 * silently drops events. A contract test asserts the shape on both sides.
 */
